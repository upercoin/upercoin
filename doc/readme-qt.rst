Upercoin-qt: Qt4 GUI for Upercoin
===============================
...
Alternatively, install Qt Creator and open the `upercoin-qt.pro` file.

An executable named `upercoin-qt` will be built.
...
To use UPnP for port forwarding behind a NAT router (recommended, as more connections overall allow for a faster and more stable upercoin experience), pass the following argument to qmake:
...
A warning for people using the *static binary* version of Upercoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Upercoin is linked against libdb5.3.
...
installed causes upercoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
